...
- [x] Capital carries day to day — one continuous career from $10,000 across an 8-week season (40 days); week position drives unlocks/missions (the calendar IS the run).
- [x] Three first-class endings: BUSTED (daily-relative 40% margin call — no run is ever safe) · SEASON CLEARED (Week 8 Day 5) · THE FUND (touch $100,000 at any moment — instant victory, the desk hands you a book).
- [x] Ending screen ×3: PIT RATING letter grade with deterministic breakdown (run P&L, win rate, max drawdown, missions), full-run replay strip (equity curve + entries/exits/events), August send-off (roast/salute), RUN IT BACK dominant.

## S2 — THE MAP
- [x] Season strip on the mode screen and between days: Weeks 1–8 with named waypoints, current position lit, next waypoint distance copy, all-time FURTHEST REACHED ghost mark; beating the ghost = "FURTHEST YET" moment.

## S3 — DAILY PIT
- [x] One shared seeded day per ET calendar date — identical tape for every player, standalone $10K, GAME-4 mechanics; 3 attempts/day, best posts; countdown to the reset; its own TODAY board separate from career records.

## S4 — RECORDS
- [x] Panel: BEST RUN $ · FURTHEST REACHED · BEST PIT RATING · BEST DAILY RANK · daily streak — persisted on the anonymous pid, claim-ready (unchanged pattern).

## S5 — MODE SELECT
- [x] PIT tab lands on a fast chooser: CAREER (continue run — persisted across reloads — or start new; map + ghost) · DAILY PIT (attempts left, countdown, board) · RECORDS. One tap to playing.

## VERIFICATION
- [x] Programmatic paths: BUSTED, WEEK-8 CLEAR, $100K FUND — endings + deterministic rating breakdowns (pure fns tested; screens click-through-verified).
- [x] Daily determinism (same date ⇒ identical tape), attempt cap 3 enforced, board takes best-of-three only.
...
- [x] Full change-nothing audit: every route/tab/component/store/API call; mock vs live; dead buttons; loading/empty/error states; mobile; console; performance. Written report (broken/placeholder/fake/duplicated/confusing, per surface) delivered BEFORE any build. Named-but-absent features recorded "not present", never created.
- [x] Home-brief hierarchy: MARKET REGIME apex → indexes (existing pulse) → desk line → latest ingest → headlines → watching; progressive disclosure, nothing added for its own sake.
- [x] MARKET REGIME (CALCULATED): held inputs only — index trend from pulse series, VIX level+trend, desk book bias, NQ vs live-book stated levels → RISK ON / NEUTRAL / RISK OFF + "because" list w/ live values; confidence % only from a real formula; never advice.
- [x] DATA INTEGRITY LABELS wherever a market number renders: LIVE / DELAYED / SIMULATED / PROXY / UNAVAILABLE; absence says DATA UNAVAILABLE, never a bare dash.
...
- [x] ANALYST: existing chat grounded — market answers cite the app's actual current data (regime/pulse/levels/book/headlines); unavailable stated unavailable; live numbers never fabricated.
...
- [x] F1 LAYOUT: regime apex top · slim ASK BAR under it (HomeBrief askBar slot; expands to full chat on engage, threads sidebar unchanged) · orb shrinks to hl-orb-mini header mark · modules per DESIGN_LAWS, DataTag on every number.
- [x] F2 COUNTDOWN ROW: lib/calendar-feed.ts + /api/calendar (High-impact or big-four USD; rate-limited; s-maxage=300) → CountdownRow. Tests 4.
  - [x] F2-A THREE STATES: distant = slim muted quiet-line strip (this real week renders it: FOMC minutes ~66h out) · imminent <48h = full card w/ forecast/prior/impact/ask-August · released ≤12h = reaction15m line ONLY when NQ=F bars honestly cover both ends; no `actual` in feed ⇒ "actuals aren't carried by the free feed", never a fabricated beat/miss.
  - [x] F2-B BRIDGE RULE: every card carries ASK AUGUST; deskChip ("desk is positioned") when the live book holds an index ticker → Terminal link.
  - [x] F2-C QUIET-DAY HERO: strip collapses to one line; regime/heatmap/feed/desk lead the page (evidence shot shows exactly this state).
- [x] F3 HEATMAP: SectorHeatmap from already-fetched SPDR sectors (color=day%, DELAYED tag); tap → single-symbol quote+spark context + ask-August. No fan-out.
- [x] F4 THE FEED: headline cards w/ publisher chip · age · TICKER_WHITELIST extracted chips · ASK AUGUST prefill. AI gist OMITTED — the brief pipeline produces no per-headline gist (gist was conditional on "where the pipeline already produces one"; it doesn't — recorded, not faked).
- [x] F5 EARNINGS STRIP: Finnhub verified on production — earnings NULL on the keyed tier → ships EARNINGS DATA UNAVAILABLE per spec. Report entry under data still required.
...
# QUEUED — TIERS-1: THE DAILY DESK (ordered 2026-08-16; one gate)
SEQUENCE NOTE (recorded at queue time): the order says "branch feature/tiers-1 off main after feature/screener-1 merges" — no feature/screener-1 exists on local or origin and no SCREENER-1 order is on record. Held for owner sequencing.
CONCEPT: LIVE book reorganizes by freshness (approval/refresh timestamps). Nothing deleted or hidden by aging — tiers are presentation over the same live book. TRACKED + performance history unchanged.
- B1 THE TIERS (computed, never managed): TODAY'S DESK (approved/refreshed this trading day; hero box atop LIVE, full cards, uncapped, "N NEW TODAY" mirrored on front-page DESK line) · THIS WEEK (current trading week not today; standard rows, 7 visible + SHOW ALL (n), visible count env-configurable default 7) · EARLIER (older, still live; collapsed to "EARLIER · n LIVE · SHOW" header, STALE chips per existing threshold, groom prompts live here). UPDATE/dedupe refresh re-enters TODAY (existing age-reset); closing/invalidating works from any tier.
- B2 EVERYWHERE: rail mirrors the three tiers (TODAY expanded, WEEK compact 7, EARLIER behind header); mobile identical; front-page DESK card + PIT desk-teaser pull TODAY first, fall back to THIS WEEK ("no new calls yet today · N live this week" — honest, never padded); filters operate across tiers, counts unchanged, API returns everything (tiers are visual pacing, never a data filter).
- B3 WEEK BOUNDARY: trading week = Mon–Fri ET, rolls midnight ET Monday; unit test w/ Friday-approval + Monday-render case.
### GATE TIERS — preview + screenshots (terminal w/ three tiers populated incl. one EARLIER expanded w/ STALE chip, rail mirror, phone, front-page "N new today"). HOLD, then merge.
