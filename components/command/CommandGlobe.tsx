...
export default function CommandGlobe({ active, flyTo }: Props) {
  ...
  // shell's toggle re-stamps it). Deliberately NOT gated by the repaint pause
  // below — a theme flip while the slide is parked must still be recorded, so
  // the basemap/scene is already correct when the World comes back into view.
  useEffect(() => {
